...
**План первого запуска на TW:** (1) `migrate`, (2) dry-run и сохранить `sha256`, (3) `--apply --expected-sha256 …`, (4) проверить `onec_client_import_runs` и `SELECT COUNT(*) FROM onec_clients`. **Откат:** повторный apply с предыдущим полным файлом (если сохранён вне приложения) или ручная корректировка; CLI **не удаляет** клиентов, отсутствующих в новом файле.
